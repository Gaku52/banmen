...
- タスク本文は**端末外へ送信しない**（全Phase共通）
- MVP (Phase 1) は通信先なし、ローカル完結
- Phase 2 以降に計測を導入する場合も、送信対象はイベント名・時刻・匿名IDのみ
- 匿名IDはランダム生成し、メール/電話番号等のPIIとは紐付けない

## 7.2 障害時フォールバック

- `localStorage` 読み込み失敗時:
  - 破損データを `banmen.state.v1.corrupt.<timestamp>` へ退避
  - `banmen.state.v1` は初期化して起動を継続
- 保存失敗（容量不足/ブラウザ制限）時:
  - そのセッション中はメモリ状態で操作継続
  - 「このブラウザでは保存できません」通知を1回表示
- 複数タブ同時利用時:
  - `storage` イベントで他タブ更新を検知
  - 最終更新時刻が新しい状態を採用（last-write-wins）
...
- `prefers-reduced-motion` 時は完了アニメーションを最小化
- 本文/背景コントラスト比は WCAG AA を満たす
- 主要操作（追加、昇格、完了、全消去）はキーボード操作可能
- スクリーンリーダーで NOW -> NEXT -> 操作群の順に読まれる
...
同期/アカウント導入の条件:
- `D7 >= 30%` を4週連続で満たす
- MVP主要導線クラッシュ率が許容範囲内（運用定義値以下）
- 上記を満たすまでは、同期/アカウントは実装しない
